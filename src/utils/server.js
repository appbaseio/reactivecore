	...
	return (App, queryString = '', ssrRenderFunc) => {
		try {
			...
			if (!storeReference) {
				...
				const output = ssrRenderFunc(App({ contextCollector }));
				let promiseFunc;
				if (!output.then) {
							...
							components
								...
								.forEach((componentId) => {
									...
									if (query && !!Object.keys(query).length) {
										...
										if (
											[queryTypes.range, queryTypes.term].includes(componentToTypeMap[
